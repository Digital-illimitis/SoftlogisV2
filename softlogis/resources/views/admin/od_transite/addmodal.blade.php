                                    ...
                                    <div class="col-4">
                                        <select class="form-select selectedProductUuid" data-placeholder="Choisir article" name="product_uuid[]">
                                            ...
                                            @foreach ($sourcing->products as $item)
                                                @php
                                                    // Récupération du libellé directement dans la vue
                                                    $libelle = optional(\App\Models\ArticleFamily::where('uuid', $item->product->famille_uuid)->first())->libelle;
                                                @endphp
                                                <option value="{{ $item->product->uuid }}" data-uuid="{{ $item->product->uuid }}" data-famille="{{ $item->product->famille_uuid }}" data-libelle="{{ $libelle }}">{{ $item->product->numero_serie ?? '--' }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="col-3">
                                        <input type="text" class="form-control px-0 text-center nature" placeholder="" disabled>
                                    </div>
                                    <div class="col-2">
...
<script>
  $(document).ready(function () {
    $(".selectedProductUuid").on("change", function () {
        var selectedOption = $(this).find(":selected");
        var libelle = selectedOption.data("libelle"); // Récupère le libellé

        // Met à jour l'input avec le libellé
        $(this).closest(".col-4").next(".col-3").find(".nature").val(libelle);
    });
});
