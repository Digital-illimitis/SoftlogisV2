                ...
                <form action="{{ route('admin.od_livraisons.store', $sourcing->uuid) }}" method="POST" enctype="multipart/form-data" class="submitForm row">
                    ...
                    <div class="row col-12">
                        ...
                        <div class="row col-12 my-3 d-flex justify-content-between">
                            ...
                            <div class="col form-group">
                                <label for="trajetEnd_uuid" class="form-label border-0 text-uppercase size_14">Point d'arrivée </label>
                                <select name="trajetEnd_uuid" class="form-select size_13" id="">
                                    <option value="" disabled selected>Selectionnez un point d'arrivée</option>
                                    @foreach ($destinations as $item)
                                        <option class="form-control" value="{{ $item->uuid }}">{{ $item->libelle ?? '--' }}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="my-3 mx-0">
                            <div class="d-flex align-items-center justify-content-between">
                                <button id="add_doc" type="button"
                                    class="add_new_box_product_import btn btn-outline-primary py-1 my-auto mb-3"><i
                            ...
                            <div class="add_new_product_import">
                                <div class="row add_new_prod gx-1 px-0 mb-3" id="0">
                                    ...
                                    <div class="col-3">
                                        <select class="form-select selectedProductUuid_import" data-placeholder="Choisir article" name="product_uuid[]">
                                            <option></option>
                                            @foreach ($sourcing->products as $item)
                                                <option 
                                                    value="{{ $item->product->uuid }}" 
                                                    data-uuid="{{ $item->product->uuid }}" 
                                                    data-longueur="{{ $item->product->longueur }}" 
                                                    data-largeur="{{ $item->product->largeur }}" 
                                                    data-hauteur="{{ $item->product->hauteur }}" 
                                                    data-poid-tonne="{{ $item->product->poid_tonne }}" 
                                                    data-designation="{{ $item->product->familly->libelle }}">

                                                    {{ $item->product->numero_serie ?? '--' }}
                                                </option>
                                            @endforeach
                                        </select>
                                    </div>
                                    ...
                                    <div class="col-1">
                                        <input type="text" class="form-control px-0 text-center longueur" placeholder="0" disabled>
                                    </div>
                                    <div class="col-1">
                                        <input type="text" class="form-control px-0 text-center largeur" placeholder="0" disabled>
                                    </div>
                                    <div class="col-1">
                                        <input type="text" class="form-control px-0 text-center hauteur" placeholder="0" disabled>
                                    </div>
                                    <div class="col-1">
                                        <input type="text" class="form-control px-0 text-center poids" placeholder="0" disabled>
                                    </div>
                                    <div class="col-1 ">
                                        <button type="button" class="btn btn-outline-danger border border-1 border-danger sup_new_box_doc_import" id=""><i class="bx bx-trash"></i></button>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="col-12 mb-4">
                            <div class="d-flex align-items-center justify-content-between">
                                <button id="add_do" type="button" class="btn btn-outline-primary py-1 my-auto mb-3" onclick="addNewDocLivraison()"><i class="bx bxs-plus-square"></i>Ajouter un document</button>
                            </div>
...
<script>
    ...
    const addButton = document.getElementById('add_doc'); // Bouton pour ajouter une marchandise
    const container = document.querySelector('.add_new_product_import'); // Conteneur des blocs
    let idCounter = 1; // Compteur pour les IDs uniques

    addButton.addEventListener('click', () => {
        // Sélectionner le premier bloc à cloner
        const originalBlock = document.querySelector('.add_new_prod');
        if (originalBlock) {
            // Cloner le bloc
            const newBlock = originalBlock.cloneNode(true);

            // Mettre à jour l'ID du nouveau bloc
            ...
            newBlock.querySelectorAll('input, select').forEach((input) => {
                if (input.tagName === 'SELECT') {
                    input.selectedIndex = 0; // Réinitialiser les sélections
                } else if (input.type === 'hidden') {
                    // Ne pas modifier les champs cachés
                } else {
                    input.value = ''; // Vider les champs de saisie
                }
            });

            // Ajouter un événement pour le bouton de suppression
            const deleteButton = newBlock.querySelector('.sup_new_box_doc_import');
            deleteButton.addEventListener('click', () => {
                newBlock.remove();
            });

            // Ajouter le nouveau bloc au conteneur
...
<script>
    ...
    // Écouteur pour le conteneur afin de gérer les événements des blocs ajoutés dynamiquement
    container.addEventListener('change', (event) => {
        const target = event.target;

        // Vérifier si l'élément sélectionné est une liste déroulante avec la classe "selectedProductUuid_import"
        if (target.classList.contains('selectedProductUuid_import')) {
            const selectedOption = target.options[target.selectedIndex]; // Option sélectionnée

            // Récupérer les données de l'option sélectionnée
            const designation = selectedOption.dataset.designation || '';
            const longueur = selectedOption.dataset.longueur || '0';
            const largeur = selectedOption.dataset.largeur || '0';
            const hauteur = selectedOption.dataset.hauteur || '0';
            const poids = selectedOption.dataset.poidTonne || '0';

            // Trouver le parent (bloc) associé à cette sélection
            const parentBlock = target.closest('.add_new_prod');

            // Mettre à jour les champs correspondants dans le parent
            if (parentBlock) {
                parentBlock.querySelector('.designation').value = designation;
                parentBlock.querySelector('.longueur').value = longueur;
                parentBlock.querySelector('.largeur').value = largeur;
                parentBlock.querySelector('.hauteur').value = hauteur;
                parentBlock.querySelector('.poids').value = poids;
            }
        }
    });
});
